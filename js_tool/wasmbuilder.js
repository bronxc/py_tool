...
function genRandomData() {
        let junk = [];
        let junkSize = getRandomInt(0,10000000);
        for (let i=0; i< junkSize; i++)
        {
            junk.push(String.fromCharCode(getRandomInt(0,0x100)));
        }
        return junk;
}
...
function constructStart(self) {
        ...
        if (funcTable[startId].type != kSig_v_v) // start function signature must be sig_v_v
          return;
...
function constructGlobal(self){    // Todo : Global init is 0 ,and can not set
...
function constructFunction(self) {
        ...
        func.exportFunc()    // export every function
}
...
function constructImportFunc(self) {
        ...
        funcTable.push({'name':importFnName, 'type':sig })
...
function constructImportTable(self) {
        ...
        self.setFunctionTableLength(initial);
        gTableLength = length;
        hasTable = true;
}
...
function constructExportFunc(self) {
    let index = getRandomInt(0,funcTable.length);
    self.addExport(funcTable[index].name, index);
}

function constructExportGlobal(self) {
    let index = getRandomInt(0,globalTable.length);
    self.addExportOfKind('exGlobal'+ (gExGlobal++), kExternalGlobal,index);
}

function constructExportTable(self) { // Table & Memory
    let index = 0;
    self.addExportOfKind('exTable', kExternalTable, index);
}

function constructExportMemory(self) {
    let index = 0;
    self.addExportOfKind('exMemory', kExternalMemory, index);
}

function constructExport(self) {
    switch(getRandomInt(0,3)) {
        case 0: constructExportFunc(self);break;
        case 1: constructExportGlobal(self);break;
        case 2: constructExportTable(self);break;
        case 3: constructExportMemory(self);break;
    }
}

function constructDate(self) {
    let isGlobal = getRandomInt(0,1);
    if (isGlobal)
        let addr = getRandomInt(0,globalTable.length);
    else
        let addr = getRandomInt(0,16384*0x1000);
    let data = genRandomData();
    self.addDataSegment(addr, data, isGlobal);
}

function constructElement(self) {
    if( !hasTable )
        return;
    let isGlobal = getRandomInt(0,1);
    let isImport = getRandomInt(0,1);
    if (isGlobal)
        let base = getRandomInt(0,globalTable.length);
    else
        let base = getRandomInt(0,gTableLength);  // table current length
    let segLength = getRandomInt(0, 10000000);  // MAX table size 10000000
    let initValueArray = [];
    for (let i =0; i< segLength; i++)
    {
        initValueArray.push(getRandomInt(0, funcTable.length));
    }
    self.addFunctionTableInit(base, isGlobal, initValueArray, isImport)
}

function constructTable(self) {
    if (hasTable)
      return;
    let size = getRandomInt(0, 10000000);
    gTableLength = length;
    self.setFunctionTableLength(length);   // Table initial ,maximum equal initial
    hasTable = true;
}
...
const constructors = ['addStart','addMemory','addExplicitSection','addCustomSection','addType','addGlobal','addFunction','addImport','addExport','addDataSegment','addElementSegment'];
...
var gFuncCount = 0;
var gImportCount = 0;
var gExGlobalCount = 0;
var gTableLength = 0;

for( var cc = 0; cc < getRandomInt(1, 1000); cc++)
{
        componentIndex =  getRandomIntInclusive(0,12);
        ...
        switch(componentIndex)
        {
                ...
                case 2:
                        ...
                        break;
                case 3:
                        //constructCustomSection(builder) // addCustomSection(name, bytes)  Chakra have not parse this section
                        break;
                case 4:
                ...
                case 7:
                        ...
                        break;
                case 8:
                        constructExport(builder) // addExport(name, index); addExportOfKind(name, kind, index)
                        break;
                case 9:
                        constructData(builder); // addDataSegment(addr, data, is_global = false)
                        break;
                case 10:
                        constructElement(builder)  // addFunctionTableInit(base, is_global, array, is_import = false)
                        break;
                case 11:
                        constructTable(builder) // setFunctionTableLength(length)
                        break;
        }
}
