...
class DisasmFunction:
    ...
    def __init__(self,address):
        ...
        self.function_name = self.get_func_name(address)
    ...
    def get_func_name(self, address):
        name = idc.Demangle(idaapi.get_func_name(address), idc.GetLongPrm(idc.INF_LONG_DN))
        if name != None:
    ...
    def resolve_all_block_by_function(self):
        ...
        while len(resolve_address_list)>0:
            ...
            if block != None:
                ...
                #获取当前block引用的所有块地址
                block_last_address = block.disasm_list[-1].address
                for ref_addr in CodeRefsFrom(block_last_address,1):
                    if ref_addr not in resolve_address_list:
                        resolve_address_list.append(ref_addr)
                ...
                if block.disasm_list[-1].mnemonics == 'jmp':
                    ...
                    #解决 jmp ecx; 这样的情况
                    if block.disasm_list[-1].operands_type[0] != idc.o_reg:
                        if jmp_to_address not in resolve_address_list:
                ...
                #需要解析跳转地址和block之后的地址
                    ...
                    if block.disasm_list[-1].operands_type[0] != idc.o_reg:
                        ...
                        if jmp_to_address not in resolve_address_list:
                            resolve_address_list.append(jmp_to_address)
                        if not_jmp_address not in resolve_address_list:
    ...
    def resolve(self):
        ...
        for block_addr in block_addr_list:
            ...
            block_last_address = block.disasm_list[-1].address
            for ref_addr in CodeRefsFrom(block_last_address,1):
                if ref_addr in block_addr_list:
                    block.sub_block_addr_list.append(ref_addr)
                    block.sub_block_dict[ref_addr] = block_dict[ref_addr]

            #添加子节点
            if block.disasm_list[-1].mnemonics != 'jmp':
                next_block_addr = block.start_address+block.size
                if next_block_addr in block_addr_list:
                    if next_block_addr not in block.sub_block_addr_list:
                        block.sub_block_addr_list.append(next_block_addr)
                        block.sub_block_dict[next_block_addr] = block_dict[next_block_addr]

            #添加子节点
            ref_from_block_address = block.disasm_list[-1].operands_value[0]
            if ref_from_block_address in block_addr_list:
                if block.disasm_list[-1].operands_type[0] != idc.o_reg:
                    if ref_from_block_address not in block.sub_block_addr_list:
                        block.sub_block_addr_list.append(ref_from_block_address)
                        block.sub_block_dict[ref_from_block_address] = block_dict[ref_from_block_address]
