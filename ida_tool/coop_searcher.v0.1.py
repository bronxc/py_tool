# V1.0 Raw 
# Use drawback to find Loop
# Assume every block have at most two ref
# 
...
class BlockNode:
    ...
    # Set next block node in normal flow 
    def set_flow_ref(node):
        self.flow_ref = node
    
    # Set next block node out of flow
    def set_jmp_ref(node):
        self.jmp_ref = node
...
def enumerate_blocks(function_ea):
    ...
    for (src_ea, dest_ea) in edges:
        ...
        if(src_node.end_ea == PrevHead(dest_node.start_ea, f_start)):
            src_node.flow_ref = dest_node
        else :
            src_node.jmp_ref = dest_node
            
    return block_nodes,edges
             

# This is an agressive algrithm, to find the first find path          
def path_to(frm_node,to_node):
    visited = set([BADADDR])
    temp_stack = []
    cursor = frm_node
    last_cursor = BADADDR
    while len(temp_stack) or cursor:
        while  cursor:
            visited.add(cursor)
            if to_node == cursor:
                return visited
            if cursor in temp_stack:
                return None
            temp_stack.append(cursor)
            cursor = cursor.flow_ref
        cursor = temp_stack.pop()
        visited.remove(last_cursor)
        last_cursor = cursor
        cursor = cursor.jmp_ref
    return None
'''   
def find_path_to(frm_node,to_node):
    def find_path_to_(frm_node,to_node, Visited):
      ...
      if frm_node == to_node:
        return [frm_node]
      if frm_node.flow_ref:
        l = find_path_to_(frm_node.flow_ref,to_node, Visited)
        if l:
          l.append(frm_node)
          return l
      if frm_node.jmp_ref:
        l = find_path_to_(frm_node.jmp_ref,to_node, Visited)
        if l:
          l.append(frm_node)
          return l
      return None
...
def find_loop_blocks(func_ea):
    ...
    if len(blocks) > 20:
        return None
    for block in blocks:
        loop_blocks = find_path_to(block.flow_ref,block)
        if loop_blocks:
            return loop_blocks
        loop_blocks = find_path_to(block.jmp_ref,block)
        if loop_blocks:
            return loop_blocks
'''           
            
# This is a agressive algrithm, to find the first loop blocks
def find_loop_blocks(func_ea):
    blocks,edges = enumerate_blocks(func_ea)
    if len(blocks) > 20:
        return None
    for block in blocks:
        loop_blocks = path_to(block.flow_ref,block)
        if loop_blocks:
            return loop_blocks
        loop_blocks = path_to(block.jmp_ref,block)
        if loop_blocks:
            return loop_blocks
